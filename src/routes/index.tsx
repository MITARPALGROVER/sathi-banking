    ...
    <main className="relative min-h-dvh overflow-x-hidden bg-background text-foreground">
      ...
      <header className="relative z-20 mx-auto flex w-full max-w-[1400px] items-center justify-between px-6 py-5 sm:px-10 sm:py-7">
        <div className="flex items-center gap-2">
          ...
          <span className="font-display text-[15px] font-semibold tracking-tight">{t("app.name")}</span>
        </div>
        <nav className="flex items-center gap-3 sm:gap-6">
          <a href="#features" className="hidden text-[11px] font-medium uppercase tracking-[0.18em] text-foreground/70 hover:text-foreground sm:inline">
            Features
          </a>
          <a href="#trust" className="hidden text-[11px] font-medium uppercase tracking-[0.18em] text-foreground/70 hover:text-foreground sm:inline">
            Trust
          </a>
          <a href="#voice" className="hidden text-[11px] font-medium uppercase tracking-[0.18em] text-foreground/70 hover:text-foreground sm:inline">
            Voice
          </a>
          <button
            type="button"
      ...
      <section className="relative z-10 mx-auto grid w-full max-w-[1400px] gap-12 px-6 pb-24 pt-6 sm:px-10 lg:grid-cols-[1.15fr_1fr] lg:items-center lg:pb-32">
        <div>
          ...
          >
            Send money, pay bills, understand your balance — by tapping, or just by talking. In Hindi, Hinglish, Punjabi, Bengali, Tamil or English.
          </motion.p>
          ...
          >
            ...
            <div role="group" aria-label="Choose your language" className="mt-4 flex flex-wrap items-center gap-2.5">
          ...
          >
            ...
            <Link to="/app" className="text-[13px] font-medium uppercase tracking-[0.18em] text-foreground/60 hover:text-foreground">
              Skip intro
            </Link>
          </motion.div>
        </div>
      ...
      {/* Features */}
      <section id="features" className="relative z-10 mx-auto w-full max-w-[1400px] px-6 py-24 sm:px-10">
        <p className="text-[11px] font-medium uppercase tracking-[0.22em] text-foreground/55">◆ Everything you need</p>
        <AccentText
          as="h2"
        ...
        <div className="mt-14 grid gap-4 md:grid-cols-3">
          {[
            ...
            { t: "History & cards", d: "Search, filter, freeze your card, and know where your money went." },
          ].map((f, i) => (
            <motion.article
              key={f.t}
              initial={{ opacity: 0, y: 12 }}
      ...
      {/* Voice */}
      <section id="voice" className="relative z-10 mx-auto w-full max-w-[1400px] px-6 py-24 sm:px-10">
        <div className="grid gap-12 md:grid-cols-12">
          <motion.p {...reveal} className="text-[11px] font-medium uppercase tracking-[0.22em] text-foreground/55 md:col-span-3">
            ◆ Voice-first
          </motion.p>
          <motion.div {...reveal} transition={{ ...spring, delay: 0.08 }} className="md:col-span-9">
            ...
            <p className="mt-8 max-w-lg text-base leading-relaxed text-foreground/70 sm:text-[17px]">
              Talk to Sathi in Hindi, Hinglish, English — even mid-sentence code-switching. Powered by an on-device LLM, we understand intent, never single keywords. Every money-moving action is confirmed twice — voice never sends by itself.
            </p>
      ...
      {/* Trust */}
      <section id="trust" className="relative z-10 mx-auto w-full max-w-[1400px] px-6 py-24 sm:px-10">
        <div className="grid gap-12 md:grid-cols-12">
          <motion.p {...reveal} className="text-[11px] font-medium uppercase tracking-[0.22em] text-foreground/55 md:col-span-3">
            ◆ Trust &amp; Safety
          </motion.p>
          <motion.div {...reveal} transition={{ ...spring, delay: 0.08 }} className="md:col-span-9">
            ...
            />
            <p className="mt-8 max-w-lg text-base leading-relaxed text-foreground/70 sm:text-[17px]">
              End-to-end encrypted. UPI PIN before every transfer. Freeze your card in one tap. No transaction ever happens on a single voice command — Sathi always asks you to confirm.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <footer className="relative z-10 mx-auto flex w-full max-w-[1400px] items-center justify-between border-t border-foreground/10 px-6 py-8 text-[11px] uppercase tracking-[0.18em] text-foreground/50 sm:px-10">
        <span>© Sathi 2026</span>
        <span>Made for Bharat · Demo, no real money</span>
      </footer>
    </main>
  );
}
